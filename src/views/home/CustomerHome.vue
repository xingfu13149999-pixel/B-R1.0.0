<!--
  客户首页（路由 name: Home，path: /）：公司信息头部 + 指标 + CustomerOnepage 一页纸卡片。
  数据来自 mock/customerViewData，与左侧树选中客户 id 关联。
-->
<template>
  <div class="home-page">
    <div class="content-panel">
        ...
        <div class="company-header">
          ...
          <div class="company-info">
            ...
            <div class="company-metrics">
              <span v-for="(m, idx) in customerHome.metrics" :key="m.label + idx" class="metric"
      ...
      <div class="content-body">
        <div class="content-left">
          <CustomerOnepage :customer-id="selectedCustomerId" />
        </div>

        <div class="content-divider" aria-hidden="true"></div>

        <aside class="content-right">
          <el-scrollbar class="content-scrollbar content-scrollbar--right">
...
import { getCustomerHomeView } from '@/views/home/mock/customerViewData'
import { useAppStore } from '@/stores/app'
import CustomerOnepage from './components/CustomerOnepage.vue'

/** 使用 public 路径，开发与打包后均稳定（不经过 import 资源解析） */
...
const { selectedCustomerId } = storeToRefs(appStore)
const findCustomerName = inject<(id: string) => string>('findCustomerName', () => '')

const companyName = computed(() => {
  const id = selectedCustomerId.value
  if (!id) return '客户'
  return findCustomerName(id) || '客户'
})

const customerHome = computed(() => getCustomerHomeView(selectedCustomerId.value))
...
.content-scrollbar :deep(.el-scrollbar__wrap::-webkit-scrollbar) {
  display: none;
  width: 0;
  height: 0;
}

.content-scrollbar :deep(.el-scrollbar__bar) {
  opacity: 0;
  transition: opacity 0.2s ease;
}

.content-scrollbar:hover :deep(.el-scrollbar__bar),
.content-scrollbar :deep(.el-scrollbar__bar:hover) {
  opacity: 1;
}
...
.content-scrollbar :deep(.el-scrollbar__thumb) {
