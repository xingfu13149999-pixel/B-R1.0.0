      ...
      >
        ...
        <div class="interview-card__tail" @click.stop>
          <el-dropdown
            ...
            @visible-change="(visible: boolean) => handleMoreVisibleChange(visible, r.id)"
            @command="(cmd: string | number) => onCardCommand(cmd, r)"
          >
            <button
              type="button"
              class="interview-card__more"
              :class="{ 'interview-card__more--active': openMoreId === r.id }"
...
function handleMoreVisibleChange(visible: boolean, recordId: string) {
  openMoreId.value = visible ? recordId : openMoreId.value === recordId ? null : openMoreId.value
}
...
.interview-card__more:focus-visible {
  background: rgba(32, 54, 202, 0.1);
  outline: none;
}

.interview-card__more--active {
  background: #2036ca;
}

.interview-card__more--active:hover,
.interview-card__more--active:focus-visible {
  background: #2036ca;
}
...
.interview-card__more--active .interview-card__dot {
  background: #fff;
}
