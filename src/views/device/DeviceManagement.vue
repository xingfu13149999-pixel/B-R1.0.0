    ...
    <div class="content-panel">
      ...
      <section class="filter-card">
        ...
        <div class="filter-actions">
          ...
          <div class="field-group">
            <label class="label">设备查询：</label>
            <div class="input-box">
              <input
                v-model="draftKeyword"
                type="text"
                placeholder="请输入设备编码或用户名"
      ...
      <section class="list-card">
        <div class="table-shell">
          <table class="device-table device-table-head">
            <colgroup>
              <col style="width: 3.4072%" />
              <col style="width: 10.2215%" />
              <col style="width: 18.7535%" />
              <col style="width: 18.7535%" />
              <col style="width: 18.7535%" />
              <col style="width: 18.7535%" />
            ...
            <thead>
              <tr>
                ...
                <th>
                  <span class="th-inner">设备用户 <img :src="iconSort" alt="" /></span>
                </th>
                <th>
                  <span class="th-inner">设备状态 <img :src="iconSort" alt="" /></span>
                </th>
                <th>
                  <span class="th-inner">最近一次登录时间 <img :src="iconSort" alt="" /></span>
                </th>
                <th class="col-op">
          ...
          <div class="table-scroll">
            <table class="device-table device-table-body">
              <colgroup>
                <col style="width: 3.4072%" />
                <col style="width: 10.2215%" />
                <col style="width: 18.7535%" />
              ...
              <tbody>
                ...
                >
                  ...
                  <td>{{ row.lastLogin ?? '-' }}</td>
                  <td class="col-op"><a href="#" @click.prevent>设备解绑</a></td>
                </tr>
...
const keyword = ref('')
const draftStatus = ref<'all' | DeviceStatus>('all')
const status = ref<'all' | DeviceStatus>('all')
...
const rows = ref<DeviceRow[]>(
  Array.from({ length: 220 }, (_, index) => {
    const id = index + 1
    const offline = id % 5 === 2 || id % 9 === 7

    return {
      id,
      code: 'HALLO WORD-BUYSTAND',
      user: 'User-butter-01',
      status: offline ? 'offline' : 'online',
      lastLogin: offline ? '2025-10-12 12:20:47' : null
    }
  })
)

const draftStatusLabel = computed(() => {
  if (draftStatus.value === 'online') return '在线'
  if (draftStatus.value === 'offline') return '离线'
  return '全部'
})
...
.device-table-body tr:hover td {
  background: #edf3f7;
}

.device-table-body tr.selected td {
  background: #e9f1ff;
}
...
.device-table input[type='checkbox']:checked {
  background: #2036ca;
  border-color: #2036ca;
}
